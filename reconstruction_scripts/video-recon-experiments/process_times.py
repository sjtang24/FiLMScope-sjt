...
parser = argparse.ArgumentParser(description="Plot timing information.")
parser.add_argument("--sample_name", type=str, default = DEFAULT_VIDEO)
parser.add_argument("--after", type=int, default = 1) # we ignore frame 1 because we dont really care about the warmup step
...
if not os.path.exists(f'{args.sample_name}_durations.csv'):
    print(f"'{args.sample_name}_durations.csv' does not exist!")
            ...
            for ds in durations.keys():
                for arr in durations[ds].keys():
                    ...
                    for frame in durations[ds][arr].keys():
                        ...
                        elif args.start <= frame < args.end:
                            #input(durations[ds][arr][frame]['duration'])
                            records.append(
                                {
                                    'stop_after' : match,
                                    'downsampling' : ds,
    ...
    duration_dataset.to_csv(f'{args.sample_name}_durations.csv', index=False)
else:
    # Load and merge datasets
    duration_dataset = pd.read_csv(f'{args.sample_name}_durations.csv')
...
AFTER = args.after 
# Load and merge datasets
metrics_dataset = pd.read_csv(f'{args.sample_name}_metrics_dataset.csv')

setup_times_df = duration_dataset[duration_dataset['frame_number'].isna()]


print(metrics_dataset)
print(setup_times_df)
...
combined.fillna({'ssim': 1, 'rmse': 0}, inplace=True)
combined['total_duration'] = combined['duration']
...
combined_metrics.to_csv(f'{args.sample_name}_skip{AFTER}.csv', index=False)
...
plt.tight_layout()
plt.savefig(f"{args.sample_name}_avg-duration-rmse-skip{AFTER}-labcomp.png")
...
for filt in [None, 0]:
    ...
    plot_with_custom_labels(
        boxplots, f'Durations (sec) for Various Configurations ({args.sample_name}; skipping {AFTER} frame(s), {incl} warmup steps)',
        f'{args.sample_name}_durations-{incl}-skip{AFTER}-labcomp.png')
...
plot_with_custom_labels(barplot, f'Setup Times for Various Configurations ({args.sample_name}; skipping {AFTER} frame(s))', 
                        f'{args.sample_name}_setup-times-skip{AFTER}-labcomp.png')
