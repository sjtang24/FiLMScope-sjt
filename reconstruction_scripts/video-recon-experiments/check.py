...
num_depths = 32 # 32 for all other examples besides skull_tool_video
...
arr = 'all' if not sample_name == 'eye_video' else [13, 14, 15, 16, 19, 20, 21, 22, 25, 26, 27, 28, 31, 32, 33, 34]
config_dict = generate_config_dict(sample_name=sample_name, gpu_number=gpu_number, downsample=3,
                                   camera_set='all', use_neptune=use_neptune, #custom_image_numbers=arr,
...
for frame_number in frame_numbers:
    ...
    for i in tqdm(range(run_args["iters"])):
        ...
        _, _, _, outputs, duration, loss_values = run_manager.run_epoch(i, log and use_neptune)
        ...
        if log and not use_neptune:
            ...
            ax1.imshow(outputs["depth"].detach().cpu().squeeze()[8:-8, 8:-8], cmap='turbo')
            ax0.imshow(run_manager.reference_image.cpu().squeeze()[8:-8, 8:-8], cmap='gray')
